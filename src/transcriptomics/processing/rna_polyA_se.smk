"""
RNA-seq poly-A single-end pipeline (SE)
========================================
Runs: FastQC → TrimGalore → FastQC trimmed → FastQ Screen →
...
# ── Sample selection ───────────────────────────────────────────────────────
all_samples = sorted(
    set(glob_wildcards(os.path.join(RAW_DIR, "{sample}_R1.fastq.gz")).sample)
)
print(f"\nExperiment : {EXPERIMENT_DIR}")
print(f"Samples found ({len(all_samples)}): {all_samples}\n")
...
if config["fqc_params"]["run_fqc"]:
    rule fqc_raw:
        input:
            fastq = os.path.join(RAW_DIR, "{sample}_R1.fastq.gz")
        output:
            html = os.path.join(pre(FASTQC_RAW), "{sample}_R1_fastqc.html"),
            zip  = os.path.join(pre(FASTQC_RAW), "{sample}_R1_fastqc.zip")
        log:
            os.path.join(log(FASTQC_RAW), "{sample}_R1.log")
        conda:
...
# ── Step 2: Trimming (single-end) ─────────────────────────────────────────
if config["trimming_params"]["run_trimming"]:
    rule trimming:
        input:
            R1 = os.path.join(RAW_DIR, "{sample}_R1.fastq.gz")
        output:
            trimmed = os.path.join(pre(TRIMMED), "{sample}_R1_trimmed.fq.gz"),
            report  = os.path.join(pre(TRIMMED), "{sample}_R1_trimming_report.txt")
        log:
            os.path.join(log(TRIMMED), "{sample}_R1.log")
        params:
        ...
        shell:
            ...
            # Trim Galore typically writes *_trimmed.fq.gz and <input>_trimming_report.txt

            mv "{params.outdir}/{wildcards.sample}_R1.fastq.gz_trimming_report.txt" \
               "{output.report}"
            """
...
if config["fqc_trimmed_params"]["run_fqc"]:
    rule fqc_trimmed:
        input:
            fastq = os.path.join(pre(TRIMMED), "{sample}_R1_trimmed.fq.gz")
        output:
            html = os.path.join(pre(FASTQC_TRIM), "{sample}_R1_trimmed_fastqc.html"),
            zip  = os.path.join(pre(FASTQC_TRIM), "{sample}_R1_trimmed_fastqc.zip")
        log:
            os.path.join(log(FASTQC_TRIM), "{sample}_R1.log")
        conda:
            os.path.join(ENV_DIR, "fastqc.yml")
...
if config["fq_screen_params"]["run_fq_screen"]:
    rule fastq_screen:
        input:
            fastq = os.path.join(pre(TRIMMED), "{sample}_R1_trimmed.fq.gz")
        output:
            txt  = os.path.join(pre(FQ_SCREEN), "{sample}_R1_trimmed_screen.txt"),
            html = os.path.join(pre(FQ_SCREEN), "{sample}_R1_trimmed_screen.html")
        params:
            conf = config["fq_screen_params"]["fastq_screen_conf"]
        log:
            os.path.join(log(FQ_SCREEN), "{sample}.log")
        conda:
...
# ── Step 5a: STAR align (single-end) ──────────────────────────────────────
if config["star_align_params"]["run_star"]:
    rule star_align:
        input:
            R1 = os.path.join(pre(TRIMMED), "{sample}_R1_trimmed.fq.gz")
        output:
        ...
        log:
            os.path.join(log(STAR_ALIGN), "{sample}_star.log")
        conda:
        ...
        shell:
            r"""
            set -euo pipefail
            # Make sure output prefix dir exists (STAR requires it if you include a slash)
            ...
            # Move/rename STAR's BAM to the desired path
            mv {params.star_bam} {output.bam}
            
            # Index once
            samtools index    -@ {threads} {output.bam}           2>> {log}

            # QC stats for MultiQC
...
# ── Step 5b: Salmon quant (single-end, runs in parallel with STAR) ─────────
if config["salmon_quant_params"]["run_salmon_quant"]:
    rule salmon_quant:
        ...
        log:
            os.path.join(log(SALMON_QUANT), "{sample}_quant.log")
        conda:
...
if config["fqc_params"]["run_fqc"]:
    MULTIQC_INPUTS[FASTQC_RAW] = expand(
        os.path.join(pre(FASTQC_RAW), "{sample}_R1_fastqc.html"), sample=samples)

if config["trimming_params"]["run_trimming"]:
    MULTIQC_INPUTS[TRIMMED] = expand(
        os.path.join(pre(TRIMMED), "{sample}_R1_trimming_report.txt"), sample=samples)
    if config["fqc_trimmed_params"]["run_fqc"]:
        MULTIQC_INPUTS[FASTQC_TRIM] = expand(
            os.path.join(pre(FASTQC_TRIM), "{sample}_R1_trimmed_fastqc.html"), sample=samples)
    if config["fq_screen_params"]["run_fq_screen"]:
        MULTIQC_INPUTS[FQ_SCREEN] = (
            expand(os.path.join(pre(FQ_SCREEN), "{sample}_R1_trimmed_screen.txt"), sample=samples) +
            expand(os.path.join(pre(FQ_SCREEN), "{sample}_R1_trimmed_screen.html"), sample=samples))
